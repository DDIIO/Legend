...
legend = Token:match("(%d+)")
redis:set(legend..":VERSION",2.9)
redis:set(legend..":SUDO_ID:",SUDO_USER)
redis:set(legend..":DataCenter:",'German')
redis:set(legend..":UserNameBot:",BOT_User)
redis:set(legend..":NameBot:",BOT_NAME)
redis:hset(legend..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3legend_INSTALL","Yes")
info = {}
...
legend = Token:match("(%d+)")
our_id = tonumber(legend)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(legend..":UserNameBot:")
SUDO_ID = tonumber(redis:get(legend..":SUDO_ID:"))
SUDO_USER = redis:hgetall(legend..'username:'..SUDO_ID).username
version = redis:get(legend..":VERSION")
DataCenter = redis:get(legend..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..legend..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(legend..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(legend..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(legend..'group:add'..msg.chat_id_) then 
...
msg.TheRank = 'مـلاك'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(legend..'omet:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRank = 'قرد'
msg.Rank = 8
elseif msg.GroupActive and redis:sismember(legend..'kenod:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRank = 'امير'
msg.Rank = 8
...
local lock_bots = redis:get(legend..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(legend..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(legend..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(legend..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(legend..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(legend..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(legend..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(legend..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(legend..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(legend..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.legend then 
if msg.text and ScriptFile.ilegend then
for k, legend in pairs(ScriptFile.legend) do
local SearchText = msg.text:match(legend)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ilegend(msg,{msg.text:match(legend)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",legend.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dlegend then
if ScriptFile.dlegend(msg) == false then
return false
end
...
if Plug.legend then 
if msg.text and Plug.ilegend then
for k, legend in pairs(Plug.legend) do
local SearchText = msg.text:match(legend)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(legend..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(legend..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ilegend(msg,{msg.text:match(legend)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",legend.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dlegend then
Plug.dlegend(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source legend")
end 

end
else
print("The File Script.lua Not Runing in The Source legend")
...
function tdcli_update_callback(data)
	...
	redis:srem(legend..'users',msg.sender_user_id_)
	end
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(legend..":SUDO_ID:")
redis:hset(legend..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(legend..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(legend..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(legend..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(legend..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(legend..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(legend..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(legend..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(legend..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(legend..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(legend..'group:add'..data.chat_.id_) then
	redis:set(legend..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(legend..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(legend..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(legend..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	legend..'group:name'..k,IDS.Title,
	legend..'num_msg_legend'..k,5,
	legend..'group:add'..k,true,
	legend..'lock_link'..k,true,
	legend..'lock_spam'..k,true,
	legend..'lock_webpage'..k,true,
	legend..'lock_markdown'..k,true,
	legend..'lock_flood'..k,true,
	legend..'lock_bots'..k,true,
	legend..'mute_forward'..k,true,
	legend..'mute_contact'..k,true,
	legend..'mute_document'..k,true,
	legend..'mute_inline'..k,true,
	legend..'lock_username'..k,true,
	legend..'replay'..k,true
	)
	redis:sadd(legend..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(legend..'username:'..ID,'username',user)
	redis:sadd(legend..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(legend..'username:'..ID,'username',user)
	redis:sadd(legend..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(legend..'username:'..ID,'username',user)
	redis:sadd(legend..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(legend..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(legend..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(legend..'username:'..data.user_.id_).username
	...
	redis:hset(legend..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
